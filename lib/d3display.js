...
export default class D3Display {
    ...
    updateDatapoints = () => {
        ...
        //
        let songPositions = g.selectAll('.songPositions')
            .data(data.filter((el, i) => i <= 20))
            .join('g')
            .attr('class', '.songPositions');
        songPositions.append('circle')
            .attr('class', 'radarCircle')
            .attr('r', 4)
            .attr("cx", function (d, i) { return rScale(d.tempo) * Math.cos(angleSlice * i - Math.PI / 2); })
            .attr("cy", function (d, i) { return rScale(d.tempo) * Math.sin(angleSlice * i - Math.PI / 2); })
            .style("fill", "#360071")
            .style("fill-opacity", 0.8);
        //append text label to dots
        songPositions.append('text')
            .attr('class', 'songName')
            .style('font-size', '8px')
            .attr('text-anchor', 'right')
            .attr('dy', '1em')
            .attr('dx', '1em')
            .attr("x", function (d, i) { return rScale(d.tempo) * Math.cos(angleSlice * i - Math.PI / 2); })
            .attr("y", function (d, i) { return rScale(d.tempo) * Math.sin(angleSlice * i - Math.PI / 2); })
